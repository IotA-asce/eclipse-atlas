...
The full-page scene uses one rendered Sun light source for Earth and Moon illumination. In Time Jump, the displayed Sun and Moon are anchored to the active observer/camera every frame at their topocentric directions, and are hidden below the local horizon; the atmospheric sky dome deliberately does not render a second solar disc. The Moon uses NASA SVS's 2025 LROC global color mosaic (2K), assembled from Lunar Reconnaissance Orbiter camera data; it is an equirectangular surface map centered at 0° longitude. Credit: NASA Scientific Visualization Studio / LRO camera teams. Source: [NASA SVS CGI Moon Kit](https://svs.gsfc.nasa.gov/4720/).
